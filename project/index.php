    ...
    if (!$error) {
        try {
            if ($log) {
                ...
                $updateResult = $stmt->execute([$fio, $number, $email, $date, $radio, $bio, $check ? 1 : 0, $form_id]);
                ...
                $stmt->execute([$fio, $number, $email, $date, $radio, $bio, $check ? 1 : 0, $login, $mpass]);
